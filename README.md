...
There is a Firefox profile stored in `system_files/firefox-profile`. This is the profile used for the fullscreen Firefox instance. The .gitignore in that directory ignores a ton of files that are generated by Firefox, but aren't needed to make the profile work for our purposes. Firefox will always regenerate those files when it runs. 
...
- launch firefox in kiosk mode on startup
- try using `cage` to prevent access to the rest of the system, maybe stop using bazzite and use core-main?
- 
